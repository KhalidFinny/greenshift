		...
		return (
			<div className="p-8 text-center">
				...
				<Link to="/broker/monthly-reports">
					<Button className="mt-4 bg-[#03442C] text-white">Back to Reports List</Button>
				</Link>
			</div>
		);
	}
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-0.5">
						Monthly Monitoring Report Period: <span className="font-semibold text-foreground">{report.period}</span>
					</p>
				</div>
				...
				<CardHeader className="bg-muted/30 pb-4">
					<div className="flex flex-wrap items-center justify-between gap-2">
						...
						<p className="text-xs text-muted-foreground">
							Reported Date: {new Date(report.submittedAt).toLocaleDateString("en-US", { dateStyle: "long" })}
						</p>
					</div>
				</CardHeader>
				...
				<CardContent className="p-6 space-y-6">
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 rounded-xl bg-muted/50 p-4 text-xs">
						<div>
							...
							<p className="font-semibold text-foreground text-sm mt-0.5">{report.companyName}</p>
						</div>
						<div>
							<p className="text-muted-foreground">General Contractor / Vendor</p>
							<p className="font-semibold text-foreground text-sm mt-0.5">{report.vendorName}</p>
						</div>
						<div>
							<p className="text-muted-foreground">Reference Project ID</p>
							<p className="font-semibold text-foreground text-sm mt-0.5">{report.projectId}</p>
						</div>
					</div>

					{/* Key Metrics Breakdown */}
					<div>
						<h3 className="text-sm font-semibold mb-3">Key Performance Summary</h3>
						<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
							<div className="rounded-xl border border-border p-4 space-y-2 bg-card">
								<div className="flex items-center justify-between text-muted-foreground text-xs">
									<span>Physical Progress</span>
									<FontAwesomeIcon icon={faProjectDiagram} className="text-emerald-600" />
								</div>
								...
								<div className="w-full bg-muted rounded-full h-1.5 overflow-hidden">
									<div
										...
										style={{ width: `${Math.min(report.actualProgressPercent, 100)}%` }}
									/>
								</div>
							...
							<div className="rounded-xl border border-border p-4 space-y-2 bg-card">
								...
								<p className="text-[11px] text-muted-foreground">
									Target: {report.expectedEnergySavingsKwh.toLocaleString("en-US")} kWh
								</p>
							</div>
						...
						<Card>
							...
							<CardContent className="space-y-4 text-xs">
								<div className="flex justify-between items-center py-2 border-b border-border">
									...
									<span className="font-semibold">{report.currentMilestoneTitle}</span>
								</div>
								<div className="flex justify-between items-center py-2 border-b border-border">
									<span className="text-muted-foreground">Completed Milestones:</span>
									<span className="font-semibold">{report.completedMilestonesCount} Milestones</span>
								</div>
								<div className="flex justify-between items-center py-2 border-b border-border">
									<span className="text-muted-foreground">Actual ROI Performance:</span>
									<span className="font-semibold text-emerald-600">{report.actualRoiPerformancePercent}% (Projected Target: {report.projectedRoiPercent}%)</span>
								</div>
							</CardContent>
						</Card>
						...
						<Card>
							...
							<CardContent className="space-y-3 text-xs">
								{report.detectedRisksOrAnomalies.length === 0 ? (
									<p className="text-muted-foreground">No anomalies or significant risks detected for this period.</p>
								) : (
									<ul className="space-y-2">
										{report.detectedRisksOrAnomalies.map((risk, idx) => (
											<li key={idx} className="flex items-start gap-2 bg-emerald-50 text-emerald-950 p-2.5 rounded-lg border border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-200">
												<FontAwesomeIcon icon={faCheckCircle} className="text-emerald-600 mt-0.5 shrink-0" />
												<span>{risk}</span>
											</li>
										))}
									</ul>
					...
					<div className="rounded-xl border border-border p-4 bg-muted/30 space-y-2 text-xs">
						...
						<p className="text-[11px] text-muted-foreground italic pt-2 border-t border-border mt-3">
							Note: This report is transparently sourced from telemetry data and on-site audits to serve as reporting material for external investors holding Green Bonds.
						</p>
					</div>
				</CardContent>
			</Card>
