...
function CreateDocumentRequestModal({
	...
	const [reason, setReason] = useState("Required for solvency ratio analysis and bond underwriting review.");
			...
			<DialogContent className="max-w-lg">
				...
				<form onSubmit={handleSubmit} className="space-y-4 pt-2 text-xs">
					<div className="rounded-lg bg-muted p-3">
						<p className="font-semibold text-foreground">Target Client: {companyName}</p>
						<p className="text-muted-foreground mt-0.5">
							* In accordance with broker role boundaries, Brokers only request documents from the Client Company. If contractor/vendor data is required, the Company coordinates it.
						</p>
					</div>
				...
				<CardContent className="p-6 space-y-4">
					<div className="flex flex-wrap items-center justify-between gap-3">
						<div className="flex items-center gap-2">
							<Badge variant="outline" className="border-white/30 text-white uppercase text-xs">
								{project.workflowStatus.replace(/_/g, " ")}
							</Badge>
						</div>
						<span className="text-xs text-emerald-200">
							Assigned Date: {new Date(project.assignedAt).toLocaleDateString("en-US", { dateStyle: "medium" })}
						</span>
					</div>
					...
					<div className="grid grid-cols-2 gap-4 rounded-xl bg-white/10 p-4 sm:grid-cols-4 text-xs">
						...
						<div>
							...
							<p className="mt-1 text-sm font-bold text-white">
								{project.bondInfo.status} ({project.bondInfo.couponRatePercent}% Coupon)
							</p>
						</div>
					</div>
				</CardContent>
			...
			<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
					...
					<Card>
						...
						<CardContent className="space-y-4 text-sm">
							...
							<div className="grid grid-cols-1 gap-3 sm:grid-cols-3 text-xs rounded-lg bg-muted p-3">
								<div>
									<p className="text-muted-foreground">Internal Rate of Return (IRR)</p>
					...
					<Card>
						...
						<CardContent className="space-y-4 text-xs">
							<p className="text-muted-foreground">
								This risk assessment is derived from GreenShift's official project evaluation. Brokers can review this data for bond issuance appraisal, but cannot modify these values.
							</p>

							<div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
								<div className="rounded-lg border border-border p-3">
									<p className="text-muted-foreground">Financial Risk</p>
									<p className="mt-1 font-bold text-emerald-600">{project.riskAssessment.financialRisk}</p>
								</div>
								<div className="rounded-lg border border-border p-3">
									<p className="text-muted-foreground">Technical Risk</p>
									<p className="mt-1 font-bold text-amber-600">{project.riskAssessment.technicalRisk}</p>
								</div>
								<div className="rounded-lg border border-border p-3">
									<p className="text-muted-foreground">Implementation Risk</p>
									<p className="mt-1 font-bold text-amber-600">{project.riskAssessment.implementationRisk}</p>
								</div>
								<div className="rounded-lg border border-border p-3">
									<p className="text-muted-foreground">Environmental Risk</p>
									<p className="mt-1 font-bold text-emerald-600">{project.riskAssessment.environmentalRisk}</p>
								</div>
							</div>

							<div className="rounded-lg bg-muted p-3 text-muted-foreground">
								💡 <strong>Risk Review Notes:</strong> {project.riskAssessment.notes}
							</div>
						</CardContent>
					</Card>
						...
						<CardContent className="space-y-4 text-xs">
							{projectDocs.length === 0 ? (
								<p className="text-muted-foreground text-center py-6">
									No document requests for this project yet. Click the button above to create a request.
								</p>
							) : (
										...
										>
											<div className="flex flex-wrap items-center justify-between gap-2">
												<span className="text-muted-foreground">
													{doc.status === "REQUESTED" ? "Requested" : doc.status === "SUBMITTED" ? "Submitted" : doc.status}
												</span>
													...
													<div className="flex items-center gap-2">
														...
														<div className="flex items-center gap-2">
															...
															<Button
																...
																variant="outline"
																onClick={() =>
																	rejectDocument(doc.id, "Document does not cover the requested period.")
																}
				...
				<div className="space-y-6">
					<Card className="sticky top-6">
						<CardHeader>
							<CardTitle className="text-lg flex items-center gap-2">
								<FontAwesomeIcon icon={faFileContract} className="text-emerald-600" />
								Underwriting & Funding Status
							</CardTitle>
						</CardHeader>
						<CardContent className="space-y-4 text-xs">
							<p className="text-muted-foreground">
								Bond issuance and placement are executed by the Broker outside the GreenShift platform. The status below serves as an external milestone tracker.
							</p>

							<div className="rounded-xl bg-muted p-4 space-y-3">
								<div className="flex justify-between items-center">
									<span className="text-muted-foreground">Issuance Status:</span>
									<Badge className="bg-[#03442C] text-white uppercase">
										{project.bondInfo.status}
									</Badge>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">Target Issuance:</span>
									<span className="font-bold text-foreground">{formatRupiah(project.bondInfo.totalAmount)}</span>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">Coupon Rate:</span>
									<span className="font-bold text-emerald-600">{project.bondInfo.couponRatePercent}% / yr</span>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">Bond Tenor:</span>
									<span className="font-bold text-foreground">{project.bondInfo.tenorMonths} Months</span>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">Lead Representative:</span>
									<span className="font-semibold text-foreground">{project.bondInfo.brokerRepresentative}</span>
								</div>
							</div>

							<div className="space-y-2 pt-2 border-t border-border">
								<p className="font-semibold text-foreground">Update External Bond Status:</p>
