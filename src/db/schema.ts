...
export const users = sqliteTable(
  'users',
  {
    ...
    role: text({ enum: userRoles }).notNull().default('company'),
    name: text().notNull(),
...
export const projectDocumentsRelations = relations(projectDocuments, ({
  one,
}) => ({
...
export const riskAssessmentsRelations = relations(riskAssessments, ({
  one,
}) => ({
  project: one(projects, {
...
export const vendorMatchScoresRelations = relations(vendorMatchScores, ({
  one,
}) => ({
  project: one(projects, {
...
export const proposalRevisionsRelations = relations(proposalRevisions, ({
  one,
}) => ({
  ...
  'blueprints',
  {
    ...
    document: text({ mode: 'json' }).$type<{
      ...
      financialProjections?: {
        ...
        scenarios?: { conservative: unknown; base: unknown; optimistic: unknown }
      }
      [key: string]: unknown
    }>(),
...
export const energyForecastsRelations = relations(energyForecasts, ({
  one,
}) => ({
  project: one(projects, {
...
export const emissionReportsRelations = relations(emissionReports, ({
  one,
}) => ({
  project: one(projects, {
