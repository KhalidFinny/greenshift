...
function TableHead({ className, ...props }: ColumnProps) {
  return (
    <ColumnPrimitive
      ...
      className={cn(
        "h-12 px-3 text-left align-middle text-xs font-medium tracking-wider whitespace-nowrap text-muted-foreground uppercase [&:has([data-slot=checkbox])]:pr-0 [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
