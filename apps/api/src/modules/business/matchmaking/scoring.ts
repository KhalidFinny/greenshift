/* The matching model's tables: the five criteria a vendor pool is scored on, and
 * the provinces its location term reads. Shared by the run, the read and the seed. */

// Weights follow the vendor-selection literature (Dickson's ordering quality >
// delivery > history > price), plus a risk term: 25 / 23 / 20 / 19 / 13.
...
// A criterion every vendor scores the same on cannot separate them, so it is left
// out of the total and the rest renormalised; the read applies the same rule.
...
// Compared on the province both locations name: full marks in the project's own
// province, 70 next door on the same island, 45 elsewhere, 20 when unrecorded.
