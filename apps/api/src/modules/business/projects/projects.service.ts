...
// Turns a validated draft into a project. Scores are recomputed here, never taken
// from the client, and the summary figures derive from the submitted inputs.
export async function submitProject(
	...
	// another draft's uploads. De-duplicated, then compared against the rows found.
	...
	const project = await repository.insertProjectWithRisk(
		db,
		{
			...
			// The next move is the company's: register at the environmental registry and appoint
			// an LVV body first, so a submission lands on `registry` rather than with the body.
			status: "registry",
			...
			// The scope of work the tender is bid against: the company's requirements, which
			// the matching model also reads, and what the delivery must hand over.
			technicalRequirements: scopeEntries(body.step3.requirements),
			deliverables: scopeEntries(body.step3.deliverables),
			// Derived so the catalog and vendor surfaces have real figures: capital is the
			// funding target, energy converts MWh to kWh, carbon is the requested share of baseline.
	...
	// Verification is not the platform's to start: the company registers the project at
	// Sistem Registri and appoints the LVV body. The reminder stays until they do.
...
// The company's half of verification: registered at the registry with an LVV body
// appointed. Only a project waiting on `registry` can start; the verdict arrives out of band.
...
// The submitted project as the app reads it back. Scores and baseline are the stored
// figures, and the risk level follows the stored score, so every screen agrees.
...
// Waits without holding the isolate's CPU, which is what `waitUntil` needs. The
// executor form is deliberate: `Promise.withResolvers` is not in this project's type lib.
...
// What the environmental registry answers. The platform has no integration with it
// yet, so this stands in for the call; a real one replaces this function only.
...
// The verification body's answer, after the wait the real one takes. Order mirrors the
// real chain: registry first, then matching, so the screen opens already populated.
export async function completeLvvReview(
	...
	// Verification clears the project into matchmaking and the matching run ranks its pool
	// in the same pass. The blueprint is generated here, while the tender can still read it.
