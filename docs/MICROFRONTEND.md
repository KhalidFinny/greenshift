...
packages/
  ...
  investor/       Public obligasi dashboard (catalog)
...
Why not Module Federation? Federation splits builds and loads remote entries at runtime, which is pointless for a single Worker bundle. The same organizational benefits (isolated packages, zero merge conflicts) come from the import contract above, without the runtime cost.
...
- `layouts/role-shell.tsx`: the app shell. Sidebar (logo, nav, account) plus an invisible header (menu name, notifications, account menu). Pure UI; logic lives in `useRoleShell`
- `EmptyState`, `cn`
...
Because each role package is a clean unit with a shared contract, splitting into separate Workers later (e.g., per-role API workers) is a mechanical extraction, since the import boundaries are already drawn.
